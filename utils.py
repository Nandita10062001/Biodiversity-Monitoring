...
def get_metadata(data_folder: str) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: The dataframe with metadata.
    '''
    ...
    for file_path in all_paths:
        try:
            ...
            class_folder_name = os.path.basename(folder_path)

            # Extract camera location from file name.
            file_name_parts = file_name.split('_')
            if len(file_name_parts) >= 3:  # Ensure enough components are present.
                camera_location = file_name_parts[2]
                meta_data_list.append([camera_location, class_folder_name, file_path])
            else:
                print(f"Skipping file due to unexpected file name format: {file_name}")
